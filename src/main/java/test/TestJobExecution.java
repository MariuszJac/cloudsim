...
public class TestJobExecution {
	...
	public void testProvider() {
		...
		assertEquals(provider.requestTaskExecutionTime(task,1), 200);
		provider.requestTaskExecution(task,0);
		assertEquals(provider.getListTasks().size(), 1);
		//add task 1
		assertEquals(provider.requestTaskExecutionTime(task2,1), 600);
		...
		provider.execute(100, 2);
		provider.execute(100, 2);
		assertEquals(provider.getCurrentlyExecutedTask(), null);
	}
